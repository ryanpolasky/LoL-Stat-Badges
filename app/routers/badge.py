...
async def get_badge(
    region: str, summoner: str, tagline: str, rank_name: bool = Query(False)
):
    """
    ...
    if len(safe_summoner) > 16:
        raise HTTPException(
            status_code=400, detail="Summoner name exceeds 16 characters."
        )
    if not re.match(r"^[a-zA-Z0-9\s._-]*$", safe_summoner):
        raise HTTPException(
            status_code=400, detail="Summoner name contains invalid characters."
        )
    ...
    if not tagline.isalnum():
        raise HTTPException(
            status_code=400, detail="Tagline contains invalid characters."
        )
    ...
    try:
        ...
        logger.info(
            f"Badge successfully generated for {safe_summoner}#{tagline} in region {region}!"
        )
